...
public:
    ...
    // elements is deleted.
    float avg();
    //Function to return the average of all elements in the
    //stack.
    void print(){for(int i = 0; i < stackTop; i++){ check.out(to_string(i+1)+": ");check.out();check.out();cout << list[i] << endl;};};
    //Function to print the values in the list in ordered of
    //push
    Type max();
    //Function to print the max value in the stack
private:
...
public:
    void getInfix();
    // A function that stores the infix expression in infix
    void showInfix();
    //
};
